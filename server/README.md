...
```sh
pnpm run dev:api    # php -S on 127.0.0.1:8080, data in ./.prenoms-data
pnpm run test:api   # unit tests, then the API tests on a throwaway directory
```
...
| `PUT` | `/api/sessions/{id}/profiles/{pid}/ratings/{mode}` | `{ratings, duels}` — the per-Profile Ranking, written whole. The client computes it (`src/lib/duel.ts`); the server only stores it. |
...
```sh
ID=$(curl -s -XPOST localhost:8080/api/sessions | sed 's/.*"id":"\([^"]*\)".*/\1/')
PID=$(curl -s -XPOST localhost:8080/api/sessions/$ID/profiles \
  -d '{"name":"Alex"}' | sed 's/.*"id":"\([^"]*\)".*/\1/')
curl -XPUT localhost:8080/api/sessions/$ID/profiles/$PID/verdicts/female/Jeanne \
  -d '{"verdict":"keep"}'
curl -XPOST localhost:8080/api/sessions/$ID/profiles/$PID/ready
curl -XPOST localhost:8080/api/sessions/$ID/final/duels \
